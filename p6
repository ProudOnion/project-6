...
extern void getTimeDateRMS(void);               // Get Time & Date from the RMS Message in the Buffer
...
// ******************************************************************************************
// *******      Scrolls selective Sentences from SAM-M8Q on LCD, April-23-2023        *******
// *******  ----=----1----=----2----=----3----=----4----=----5----=----6----=----7    *******
// ****  $GNRMC,132903.00,A,3335.18464,N,10152.53799,W,0.491,,180322,,,A*7F__ (65 Chrs)  ****
// **************         PPS (Pulse per Second Pin) is routed to P1.3         **************
// ******************************************************************************************
...
//  Find Start of GPS Message / Start is the position of the '$'
        ...
        for( j = 0; j < Bufsize-1; j++ )        // Loop through the Data Buffer and find Start of GPS message
            {
             if( (GPS[j] == '$') && (GPS[j+3] == 'R') && (GPS[j+4] == 'M') && (GPS[j+5] == 'C') && (j < (Bufsize-70)) )
              {
...
// ******************************************************************************************
      if( FdStart == 1 )
      {
          getTimeDateRMS();                     // Get the Time & the Date from the RMS Message
//        showTime();                           // Show UTC Time
//        showDate();                           // Show Date
          scrollDisp();                         // Scroll GPS Message on the LCD Display
          CheckSum();                           // Process & Display Checksum
      }
...
// ******************************************************************************************
// *****    Get the Time & Date Information from the RMS Sentence in the Buffer         *****
// ******************************************************************************************
void getTimeDateRMS(void)
{
   ...
   if(GPS[i+6] == ',')                          // Confirm 1st comma after Message Header
       {
        Hour    =           10*(GPS[i+7] - 48); // 10's of Hours,   48 is ASCII Table offset
        Hour    = Hour  +      (GPS[i+8]  -48); //         Hours,   48 is ASCII Table offset
        Min     =           10*(GPS[i+9]  -48); // 10's of Minutes, 49 is ASCII Table offset
   ...
   for ( i = GPSstart; i < GPSstop; i++ )
         {
           if(GPS[i] == ',') comma = comma + 1; // Found another comma
              if(comma == 2)                    // Found position of of comma before Data Valid
              {
                  Valid =      GPS[i];          // Capture 'Data Valid' Flag (after loop advances once)
              }
              if(comma == 9)   break;           // Found position of of comma before Date; end For loop
         }
